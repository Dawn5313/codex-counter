...
`ccodexr` 当前由 [Dawn5313](https://github.com/Dawn5313) 作为主维护者继续开发和发布，公开仓库、版本发布和后续路线也都以这个仓库为准。

同时，它仍然是基于原始项目 `codexbar` 演进出来的衍生开源项目，保留了原项目的历史提交、许可证和来源关系。也就是说：

- 这个仓库的维护者和当前发布者是你
- 但原项目作者与历史贡献者不应该被从许可证和来源说明里删除
- 如果以后继续大幅重构，依然建议保留 `based on codexbar` 这一层来源说明
