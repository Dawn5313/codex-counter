...
- Do not manually edit `~/.codex/auth.json` or `~/.codex/config.toml` when ccodexr can perform the operation.
- Do not print `access_token`, `refresh_token`, or `id_token` in logs, output, or summaries.
- If low-level repair is explicitly required, mention that the normal path is the ccodexr app before editing auth/config files directly.

## 本地安装清理

- 只要本次任务涉及本地构建、安装、替换或发布 `ccodexr.app`，结束前必须做安装清理，不要留下会在 App Library、Spotlight 或 Launch Services 中表现为“多个 ccodexr”的残留。
- 默认必须清理本次任务产生或显然属于构建/安装残留的 `ccodexr.app` 副本与目录，例如仓库内 build/staging 目录、`DerivedData` 产物、`/private/tmp` 下的临时安装目录、临时挂载出的测试副本。
- 默认必须核对最终可见性：`mdfind`、`lsregister` 或等价检查应只剩目标安装副本，通常是 `/Applications/ccodexr.app`。
